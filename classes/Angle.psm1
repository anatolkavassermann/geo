<#
    Класс Angle проверяет правильность параметра, 
    ParamDataType которого указан Angle.
...
class Angle : MyData {
    ...
    hidden [void] TryConvert ()
	{
        ...
        switch (($this.FormattedData -lt 0) -or ($this.FormattedData -gt 90)) {
		    $true
		    {
			    $this.DataIsCorrect = $false
			    ShowMessageWrongConf -Section $this.SectionName -ParameterName $this.ParameterName -ConfigFilePath $this.ConfigFilePath -PatternName $null -ErrorMessage "The angle value cannot be more than 90 degrees and less than 0 degrees"
			    return;
            }
        }
		return;
