<#
    Класс Height проверяет правильность параметра, 
    ParamDataType которого указан Height.
...
class Height : MyData {
    ...
    hidden [void] TryConvert ()
	{
        ...
        switch ($this.FormattedData -lt 0) {
		    ...
		    {
			    $this.DataIsCorrect = $false
			    ShowMessageWrongConf -Section $this.SectionName -ParameterName $this.ParameterName -ConfigFilePath $this.ConfigFilePath -PatternName $null -ErrorMessage "The angle value cannot be less than 0 km"
			    return;
            }
        }
		return;
